...
public class FakeCoins {
	...
	private void setCoins(Coins c) {
		if (c != null)
			...
			throw new IllegalArgumentException(
					"The object -coins- is null, cannot be set");
	}
	...
	public int findFake() {
		if (coins.getNumberOfCoins() < 3)
			throw new IllegalStateException(
					"We cannot determine wich is the fake coins for "
							+ coins.getNumberOfCoins() + " coins.");
		calculateWeight();
	...
	/**
	 * Not working at this time. Need to find a function that splits an array in 3 equal parts.
	 * 
	 * @param left
	 *            index.
	 * @param right
	...
	private int findFake2V(int left, int right) {
		...
		if (coins.weigh(left, leftPointer - 1, rightPointer + 1, right) == ScalePosition.EQUAL) {
			return findFake2V(leftPointer, rightPointer);
				...
				.weigh(left, leftPointer - 1, leftPointer, rightPointer) == ScalePosition.EQUAL) {
	...
	private int findFake3V(int[] coinArray) {
		if (coinArray.length == 0)
			...
			int[] rightCoins = Arrays.copyOfRange(coinArray, 2 * third,
					coinArray.length);
