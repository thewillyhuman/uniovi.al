	...
	public void test() {
		for (int i = INICIAL_RUNS; i <= FINAL_RUNS; i++) { // Coins in the bag (bags between 100 and 1000 coins)
			for (int j = 0; j < i; j++) { // Position of the fake coin
				...
				int fakePos = fakeCoins.findFake(); // Our algorithm says that the fake coin is in the position fakeCoin. Lets compare the expected result (fakePos) with the actual position we know (j)
				assertEquals(
						"Error because the expected result is not equal to the obtained one:",
						j, fakePos);
			}
			System.out.println("PROCESSED: " + ((float) i - INICIAL_RUNS)
					/ (FINAL_RUNS - INICIAL_RUNS) * 100 + " %");
		}
	}

	@Test
	public void weightTest() {
		for (int i = INICIAL_RUNS; i <= FINAL_RUNS; i++) { // Coins in the bag
															// (bags between
			// 100 and 1000 coins)
			Coins bag = new Coins(i);
			FakeCoins coins = new FakeCoins(bag);
			int calculated = coins.calculateWeight();
			System.out.println("REAL: " + bag.weight + " CALCULATED: "
					+ calculated + " size = " + i);
