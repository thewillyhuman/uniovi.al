...
public class CityTest {
	...
	try {
	    ...
	    String[] size = br.readLine().split(","); // width and height of the
						      // city
	    City roads = new City(Integer.parseInt(size[0]),
		...
		if (barriers.length > 0)
			...
			String[] barrierPosition = barriers[i].split(","); // barrierX
									   // and
									   // barrierY
			roads.addObstacle(Integer.parseInt(barrierPosition[0]),
				Integer.parseInt(barrierPosition[1]));
		    }
	    ...
	    roads.setOrigin(Integer.parseInt(start[0]),
		    Integer.parseInt(start[1]));
	    roads.setDestination(Integer.parseInt(end[0]),
